        ...
        export_wizard: {
            ...
            targets: {
                unsupported_framework:
                    'Экспорт {{framework}} несовместим с СУБД диаграммы',
                sql: {
                    title: 'SQL',
                ...
                dbml: {
                    title: 'DBML',
                    description: 'Портативный файл схемы DBML',
                    coming_soon:
                        'Экспорт файла скоро будет доступен. Просматривайте и копируйте DBML на боковой панели.',
                },
                framework: {
                    coming_soon: 'Запланировано для будущего этапа экспорта.',
                },
                diagram_json: {
                    title: 'JSON',
                ...
                prisma: {
                    title: 'Prisma',
                    description: 'Экспорт схемы Prisma',
                },
                ef_core: {
                    title: 'EF Core',
                    description: 'Экспорт Entity Framework Core',
                },
                rails: {
                    title: 'Rails',
                    description: 'Экспорт схемы Ruby on Rails',
                },
                django: {
                    title: 'Django',
                    description:
                        'Экспорт приложения Django 6.1 для прямой интеграции',
                },
                drizzle: {
                    title: 'Drizzle',
            ...
            prisma: {
                unsupported_database:
                    'Экспорт Prisma недоступен для текущего типа базы данных.',
                version_step: {
                    title: 'Версия Prisma',
                    description:
                        'Выберите основную версию Prisma для экспорта schema.prisma.',
                    prisma_7: 'Prisma 7',
                    prisma_7_recommended: 'Рекомендуется',
                    prisma_6: 'Prisma 6',
                    continue: 'Продолжить',
                },
                preview_step: {
                    description: 'Просмотрите сгенерированную схему Prisma.',
                    generating: 'Генерация схемы Prisma...',
                    download: 'Скачать schema.prisma',
                    generation_error:
                        'Не удалось сгенерировать схему Prisma. Повторите попытку.',
                    empty: 'Для текущей диаграммы схема Prisma не была сгенерирована.',
                    limitations: 'Ограничения',
                    errors: {
                        unsupported_database:
                            'Экспорт Prisma не поддерживается для этого типа базы данных.',
                        empty_diagram: 'В диаграмме нет экспортируемых таблиц.',
                        invalid_primary_key:
                            'У таблицы недопустимая конфигурация первичного ключа.',
                        unsupported_structural_field:
                            'Первичный или внешний ключ использует неподдерживаемый тип поля.',
                        invalid_enum:
                            'Не удалось экспортировать определение enum.',
                    },
                    notes: {
                        view_skipped:
                            'Представления базы данных не экспортируются в схемы Prisma.',
                        schema_namespace_unsupported:
                            'Схемы/пространства имён таблиц не сопоставляются в этом экспорте.',
                        unsupported_field_omitted:
                            'Некоторые неподдерживаемые поля были пропущены.',
                        unsupported_default_omitted:
                            'Некоторые неподдерживаемые значения по умолчанию были пропущены.',
                        unsupported_index_omitted:
                            'Некоторые индексы были пропущены.',
                        relation_skipped: 'Не удалось экспортировать связь.',
                        relation_degraded:
                            'Связь экспортирована с пониженной точностью.',
                        composite_fk_unsupported:
                            'Составные внешние ключи не поддерживаются.',
                        many_to_many_label_only:
                            'Связи многие-ко-многим без соединительной таблицы экспортируются только как метка.',
                        set_null_omitted:
                            'ON DELETE SET NULL пропущен, где не поддерживается.',
                        enum_skipped: 'Не удалось экспортировать enum.',
                        composite_type_skipped:
                            'Не удалось экспортировать составной тип.',
                    },
                    notes_grouped: {
                        schema_namespace_unsupported:
            ...
            django: {
                ...
                result_step: {
                    ...
                    provider_label: 'Провайдер: {{provider}}',
                    package_type:
                        'Пакет: приложение Django для прямой интеграции (`foxaldb_models`)',
                    generating: 'Создание пакета Django…',
                    success: 'Пакет Django 6.1 создан.',
                    generated_files: 'Созданные файлы ({{count}})',
                    notes_heading: 'Примечания',
                    ...
                    download_zip: 'Скачать ZIP',
                    retry: 'Повторить',
                    error_semantic: 'Не удалось создать пакет Django.',
                    error_unauthenticated:
                        'Для экспорта пакетов Django необходимо войти в систему.',
                    error_invalid_request:
                        'Не удалось экспортировать диаграмму. Она может быть недействительной или слишком большой.',
                    error_rate_limited:
                        'Слишком много запросов на экспорт. Подождите немного и повторите попытку.',
                    error_unexpected:
                        'Экспорт Django не выполнен на сервере. Попробуйте ещё раз.',
                    error_network:
                    ...
                    errors: {
                        unsupported_database:
                            'Экспорт Django недоступен для этого типа базы данных.',
                        empty_diagram:
                            'На диаграмме нет экспортируемых таблиц.',
                        unsupported_structural_field:
                            'Поле первичного ключа на «{{path}}» нельзя представить в Django.',
            ...
            laravel: {
                options_step: {
                    ...
                    include_indexes_description:
                        'Экспортировать явные определения индексов.\nУникальные ограничения на уровне полей включаются всегда.',
