...
import type {
    DrizzleExportFailure,
    DrizzleExportSuccess,
} from '../drizzle-export-types';
...
import { exportDrizzleProject } from '../drizzle-export';
...
const success: DrizzleExportSuccess = {
    success: true,
    filename: 'sample-drizzle.zip',
...
describe('drizzle export API', () => {
    beforeEach(() => {
        apiRequestMock.mockReset();
        apiRequestMock.mockResolvedValue(success);
    });

    it('posts the canonical diagram to the Drizzle export endpoint', async () => {
        const result = await exportDrizzleProject({
            diagram: sampleDiagram,
        });

        expect(apiRequestMock).toHaveBeenCalledWith('/exports/drizzle', {
            method: 'POST',
            data: {
                diagram: sampleDiagram,
            },
    ...
    it('sends only the live diagram in the request body', async () => {
        await exportDrizzleProject({ diagram: sampleDiagram });
    ...
    it('does not send version, diagramId, provider, or options fields', async () => {
        ...
        expect(data).not.toHaveProperty('drizzleVersion');
        expect(data).not.toHaveProperty('ormVersion');
        expect(data).not.toHaveProperty('kitVersion');
    ...
    it('returns a typed success response including structured notes', async () => {
        const result = await exportDrizzleProject({ diagram: sampleDiagram });

        expect(result.success).toBe(true);
        if (result.success) {
            expect(result.filename).toBe('sample-drizzle.zip');
    ...
    it('returns semantic HTTP 200 failures without treating them as transport errors', async () => {
        const failure: DrizzleExportFailure = {
            success: false,
            error: {
        ...
        await expect(
            exportDrizzleProject({ diagram: sampleDiagram })
