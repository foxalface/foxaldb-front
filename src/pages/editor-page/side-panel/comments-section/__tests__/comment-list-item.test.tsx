...
const { authState, accessState, updateCommentMock } = vi.hoisted(() => ({
    authState: {
...
describe('CommentListItem', () => {
    ...
    it('shows Edit for owner and editor on their own comment', async () => {
    ...
    it('hides Edit for owner on another user comment in this milestone', () => {
        render(
            <CommentListItem
    ...
    it('hides the action trigger for editor other, viewer, and deleted author', () => {
        accessState.diagramAccess = {
            role: 'editor',
            can_edit: true,
            can_manage_members: false,
        ...
        const second = render(<CommentListItem comment={comment()} />);
        expect(
            screen.queryByRole('button', { name: 'Comment actions' })
        ).not.toBeInTheDocument();
        second.unmount();
    ...
    it('unmounts cleanly when the comment is removed (remote delete)', () => {
        const { unmount } = render(<CommentListItem comment={comment()} />);
        expect(screen.getByText('Persisted body')).toBeInTheDocument();
